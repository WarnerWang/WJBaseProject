...
- (void)drawInRect:(CGRect)rect withContentMode:(UIViewContentMode)contentMode clipsToBounds:(BOOL)clips{
    CGRect drawRect = [UIImage getCGRectFitWithContentMode:rect size:self.size mode:contentMode];
...
- (UIImage *)imageByRotateLeft90 {
    return [self imageByRotate:[UIImage degreesToRadians:90] fitSize:YES];
}
...
+ (CGRect)getCGRectFitWithContentMode:(CGRect)rect size:(CGSize)size mode:(UIViewContentMode)mode{
...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius
    ...
    if (self.size.width < 1 || self.size.height < 1) {
        NSLog(@"UIImage+Helpers error: invalid size: (%.2f x %.2f). Both dimensions must be >= 1: %@", self.size.width, self.size.height, self);
        return nil;
    }
    if (!self.CGImage) {
        NSLog(@"UIImage+Helpers error: inputImage must be backed by a CGImage: %@", self);
        return nil;
    }
    if (maskImage && !maskImage.CGImage) {
        NSLog(@"UIImage+Helpers error: effectMaskImage must be backed by a CGImage: %@", maskImage);
        return nil;
    }
